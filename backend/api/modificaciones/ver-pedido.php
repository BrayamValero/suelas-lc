...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($pedido_id)){
        ...
        $sql = "SELECT ID AS PROD_ID, SUELA_ID, SERIE_ID, COLOR_ID, CANTIDAD, RESTANTE, URGENTE, ESTADO FROM PRODUCCION WHERE PEDIDO_ID = ?;";
        ...
        foreach ($datosSeries as $key => $serie) {
            ...
            foreach ($grupo_series as $key => $referencia) {
                ...
                foreach ($datosPedido as $key => $pedido) {
                    if(($suela_id == $pedido['SUELA_ID'] && $color_id == $pedido['COLOR_ID']) && $pedido['ESTADO'] != 'COMPLETADO'){
                        $cantidad = $pedido['RESTANTE'];
                ...
                if($cantidad == 0){
                    ...
                    <div class='form-group col mb-0'>
                        <label class='label-cantidades' for='cantidades'>$talla</label>
                        <input type='number' name='pedido[{$i}][cantidad]' value='$cantidad' class='form-control input-cantidades mb-2'>
                        <small class='text-muted'>Restante</small>
                        <input type='hidden' name='pedido[{$i}][prod_id]' value='$prod_id'>
                    </div>";
