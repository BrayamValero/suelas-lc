...
<div id="contenido">
    ...
    <!-- Incluimos el Navbar -->
    <?php get_navbar('Ventas', 'Alimentar Pedido #' . htmlspecialchars($_GET['id'])); ?>
...
$.ajax({
type: 'post',
url: 'backend/api/utils.php?fun=obtenerPedidoAlimentarId',
                    ...
                    if (stock.SUELA_ID === result[index].SUELA_ID && stock.COLOR_ID === colorId) {
                        ...
                        datosPedido.some(elem => {
                            ...
                            if (elem.SUELA_ID == cantidades.getAttribute('data-suela-id') && elem.COLOR_ID == cantidades.getAttribute('data-color-id')) {
                                ...
                                cantidades.value =100;

                                // // Colocamos el produccción_id en los campos NECESARIOS
                                // inputStock.setAttribute("data-prod-id", elem.PROD_ID);
                                
                                // if (parseInt(stock.CANTIDAD) < parseInt(elem.RESTANTE)) {
                                
                                //     inputStock.max = stock.CANTIDAD;
                                //     cantidades.value = elem.RESTANTE;

                                // } else {

                                //     inputStock.max = cantidades.value = elem.RESTANTE;
...
document.getElementById('botonAlimentarPedido').addEventListener('click', function () {
    ...
    if(verif_disponibilidad.includes(false)){
        ...
        Swal.fire({
            title: '¿Estás seguro?',
            text: 'Estás suelas estarán disponibles inmediatamente para el cliente.',
            icon: 'warning',
            ...
            if (result.value) {
                ...
                $.post("backend/api/pedidos/alimentar-pedido.php", {'actualizar_stock' : JSON.stringify(actualizar_stock)} , function(data) {
                    ...
                    default:
                        ...
                        return Swal.fire('Success', 'SUCCESS.', 'success');
